    ...
    return (
        <>
            <form role="search" className="rediger" onSubmit={handleSearchSubmit}>
                <SearchField label="Søk etter verktøy" hideLabel={true} error={error}>
                    <SearchField.Input className={errorclass} value={value} onChange={handleSearchChange}
                                       autofocus="autofocus"/>
                    {!!value && (
            ...
            {!search && !value &&
                <>
                    {/*
                   <Button type="submit" size="xsmall" className="rediger card__micro" value="#analyse"
                            onClick={() => setSearch("True") & setValue("") & setError("")}>Kategorier</Button>
*/}
            ...
            {search && !value &&
                <>
                    <div className="rediger">
                        <div className="katalogwrapper"><Heading level="2" aria-live="polite" className="kataloglabel"
                                                                 size="medium">Kategorier</Heading></div>
                        {category.categories.map((category, i) => (
                            <div key={i}>
                                <Button type="submit" className="searchsuggestion" value={category.slug}
            ...
            {value &&
                <div className="rediger">
                    {searchActivate && value && foundUsers && foundUsers.length > 0 &&
                        <div className="katalogwrapper"><Heading level="2" aria-live="polite" className="kataloglabel"
                                                                 size="medium"> {foundUsers.length} {foundUsers.length > 1 ? (<>resultater</>) : (<>resultat</>)}</Heading>
                        </div>}
                    {value && foundUsers && foundUsers.length > 0 ? (
                        foundUsers.map((tool) => (
                            <LinkPanel key={tool.id} className="rediger" href={tool.link} target="_blank"
                        ...
                        <>
                            {value &&
                                <Alert className="rediger" variant="error">
                                    ...
                                    <p>Vi legger stadig til nye verktøy, <Link target="_blank"
                                                                               href="https://forms.office.com/r/UyFMJsLfKM?lang=nb-NO">tips
                                        oss gjerne</Link> om hva du forsøkte å søke etter, så legger vi det til.</p>
